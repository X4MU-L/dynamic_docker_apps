...
fn test_register_and_select_backend_round_robin() {
    ...
    let item1 = BackendItem::new("172.28.0.2".to_string(), 8080, Some("app-1".to_string()), None);
...
fn test_register_duplicate_returns_conflict_error() {
    let state = DynamicLBState::new(Algorithm::RoundRobin);
    let item1 = BackendItem::new("172.28.0.2".to_string(), 8080, None, None);
    assert!(register_upstream(&state, item1.clone()).is_ok());

    let err = register_upstream(&state, item1).unwrap_err();
    assert_eq!(err, DomainError::BackendAlreadyExists("172.28.0.2:8080".to_string()));
}
...
fn test_deregister_non_existent_returns_not_found_error() {
    ...
    assert_eq!(err, DomainError::BackendNotFound("172.28.0.99:8080".to_string()));
}
...
fn test_payload_validation_errors() {
    ...
    assert_eq!(invalid_port.validate().unwrap_err(), DomainError::InvalidPort(0));
}
